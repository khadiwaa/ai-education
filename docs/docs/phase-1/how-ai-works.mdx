...
|-------|--------------|----------------|
| **Disabled / Off** | No extended thinking. Standard forward pass. | Quick lookups, simple code edits, explaining a concept, formatting tasks |
...
| **Medium** | Balanced thinking budget — the default for most models | Day-to-day coding, moderate debugging, writing and editing |
| **High** | Large thinking budget — the model spends significant time reasoning | Complex bugs, architectural decisions, multi-step logic, math, anything where accuracy really matters |

:::tip Rule of thumb
Use the lowest level that gives you a good answer. High reasoning is slower and costs more tokens. For most engineering tasks, **medium is the right default**. Reach for high when you've tried medium and the answer isn't good enough.
:::
...
Use the `/model` command in your Copilot CLI session to see available models and switch between them. Models like `claude-sonnet-4.6` and `claude-opus-4.8` have reasoning effort parameters that you can set:

```
/model                          # see available models
claude-sonnet-4.6               # standard — good for most tasks
claude-opus-4.8                 # high capability, supports high reasoning effort
```

Some models surface reasoning effort as a selectable option; others bake it into the model name (e.g., OpenAI's `o4-mini` vs `o3`). The capability exists in both cases — the interface just differs.
