...
- An active GitHub Copilot subscription (through your Herbalife organization)
- For the npm install: **Node.js 22 or later**
- Terminal on macOS, Linux, or Windows (PowerShell v6+ on Windows)

### Install

Copilot CLI is a standalone agentic tool — install it directly, not as a `gh` extension. Pick whichever package manager fits your setup:
...
:::tip First-run authentication
The first time you launch `copilot`, if you're not already logged in you'll be prompted to run the `/login` slash command inside the session. Run it and follow the on-screen instructions — there's no separate `gh auth login` step.
:::
...
- [ ] Install: `npm install -g @github/copilot` (Node.js 22+) — or `brew install copilot-cli` / `winget install GitHub.Copilot`
- [ ] Verify: `copilot --version`
- [ ] Start your first session from a repo root: `cd your-repo && copilot`
- [ ] On first launch, authenticate with the `/login` slash command if prompted
